...
class GildedRose {
    ...
    private UpdatedItem updated(Item item) {
        if ("Sulfuras, Hand of Ragnaros".equals(item.name)) {
            ...
            return new UpdatedItem(new UpdatedNormalQuality(item.quality, new DoubledAdjustment(new NormalQualityAdjustment(item.sellIn))), new UpdatedNormaSellIn(item.sellIn));
        }
    ...
    private UpdatedItem updatedAgedBrie(Item item) {
        return new UpdatedItem(
            new CappedQuality(
                new UpdatedNormalQuality(item.quality,
                    new InvertedAdjustment(new NormalQualityAdjustment(item.sellIn))
                )
            ),
            new UpdatedNormaSellIn(item.sellIn));
    }
